  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-slate-900">{fp.vl2Title}</h1>
          <p className="text-sm text-muted-foreground">{fp.vl2Desc}</p>
        </div>
      </div>
      ...
      {data && (
        <>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              { label: fp.vl2SummaryOutstanding, value: data.summary.totalOutstanding, color: 'text-slate-900 font-bold' },
              ...
              { label: fp.vl2SummaryCount, value: data.summary.supplierCount, color: 'text-slate-700', isCount: true },
            ].map(c => (
          ...
          <div className="rounded-lg border bg-white overflow-x-auto">
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead className="w-24">{fp.vl2ColCode}</TableHead>
                  <TableHead>{fp.vl2ColName}</TableHead>
                  <TableHead className="text-right w-16">{fp.vl1ColCount}</TableHead>
                  <TableHead className="text-right w-32">{fp.vl2ColTotal}</TableHead>
              ...
              <TableBody>
                {data.rows.length === 0 ? (
                  <TableRow><TableCell colSpan={8} className="py-12 text-center text-muted-foreground">{fp.vl2NoRecords}</TableCell></TableRow>
                ) : (
                  data.rows.map(row => (
