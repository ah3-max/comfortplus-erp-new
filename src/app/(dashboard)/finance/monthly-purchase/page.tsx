...
interface MonthRow { month: number; count: number; totalAmount: number }
interface PurchaseData { year: number; grandTotal: number; totalOrders: number; rows: MonthRow[] }
...
export default function MonthlyPurchasePage() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true)
    try {
      const res = await fetch(`/api/finance/monthly-purchase?year=${year}`)
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-slate-900">{dict.nav.monthlyPurchase}</h1>
          <p className="text-sm text-muted-foreground">每月採購訂單金額統計</p>
        </div>
      </div>
        ...
        <>
          <div className="grid grid-cols-2 gap-3">
            <div className="rounded-lg border bg-white p-3">
              <p className="text-xs text-muted-foreground mb-1">全年採購總額</p>
              <p className="text-base font-mono font-bold text-amber-700">${fmt(data.grandTotal)}</p>
            </div>
            <div className="rounded-lg border bg-white p-3">
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="text-right w-24">採購單數</TableHead>
                  <TableHead className="text-right w-36">採購金額</TableHead>
                  <TableHead>金額佔比</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {data.rows.map(row => {
                  ...
                  return (
                    <TableRow key={row.month} className={`text-sm ${row.count === 0 ? 'text-muted-foreground' : 'hover:bg-slate-50/40'}`}>
                      ...
                      <TableCell className="text-right font-mono">{row.totalAmount > 0 ? `$${fmt(row.totalAmount)}` : '—'}</TableCell>
                      <TableCell>
                        {pct > 0 && (
                ...
                <TableRow className="border-t-2 bg-slate-50 font-semibold text-sm">
                  ...
                  <TableCell className="text-right font-mono text-amber-700">${fmt(data.grandTotal)}</TableCell>
                  <TableCell />
                </TableRow>
              </TableBody>
            </Table>
