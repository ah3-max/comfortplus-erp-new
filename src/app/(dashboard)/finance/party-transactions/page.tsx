...
export default function PartyTransactionsPage() {
  ...
  const typeLabels: Record<string, string> = {
    AR: fp.ptxTypeAR, AP: fp.ptxTypeAP,
    PAYMENT_IN: fp.ptxTypePaymentIn, PAYMENT_OUT: fp.ptxTypePaymentOut,
  }
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-slate-900">{fp.ptxTitle}</h1>
          <p className="text-sm text-muted-foreground">{fp.ptxDesc}</p>
        </div>
      </div>

      <div className="flex flex-wrap items-end gap-3 rounded-lg border bg-white p-4">
        <div className="space-y-1">
          <label className="text-xs font-medium text-muted-foreground">{fp.ptxPartyTypeLabel}</label>
        ...
        <div className="space-y-1">
          <label className="text-xs font-medium text-muted-foreground">{fp.ptxIdLabel}</label>
          <input
            type="text" value={partyId} onChange={e => setPartyId(e.target.value)}
            placeholder={fp.ptxIdPlaceholder}
      ...
      {data && (
        <>
          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              { label: partyType === 'CUSTOMER' ? fp.ptxArTotal : fp.ptxApTotal, value: data.summary.totalDebit, color: 'text-slate-900' },
              { label: partyType === 'CUSTOMER' ? fp.ptxReceiptTotal : fp.ptxPaymentTotal, value: data.summary.totalCredit, color: 'text-green-600' },
              { label: fp.ptxNetBalance, value: data.summary.netBalance, color: data.summary.netBalance >= 0 ? 'text-slate-900 font-bold' : 'text-red-600 font-bold' },
            ].map(c => (
          ...
          <div className="rounded-lg border bg-white overflow-x-auto">
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead className="w-24">{fp.ptxColDate}</TableHead>
                  <TableHead className="w-20">{fp.ptxColType}</TableHead>
                  <TableHead className="w-32">{fp.ptxColRef}</TableHead>
                  <TableHead>{fp.ptxColSummary}</TableHead>
                  <TableHead className="text-right w-28">{fp.ptxColDebit}</TableHead>
                  <TableHead className="text-right w-28">{fp.ptxColCredit}</TableHead>
                  <TableHead className="text-right w-32">{fp.ptxColBalance}</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {data.rows.length === 0 ? (
                  <TableRow><TableCell colSpan={7} className="py-12 text-center text-muted-foreground">{fp.ptxNoRecords}</TableCell></TableRow>
                ) : (
                  data.rows.map(row => {
                    const typeLabel = typeLabels[row.type] ?? row.type
                    const typeCls = TYPE_CLASS[row.type] ?? 'bg-slate-100 text-slate-600'
                    return (
                      <TableRow key={row.id} className="text-sm hover:bg-slate-50/40">
                        <TableCell className="text-muted-foreground">{row.date}</TableCell>
                        <TableCell><Badge className={`text-xs ${typeCls}`}>{typeLabel}</Badge></TableCell>
