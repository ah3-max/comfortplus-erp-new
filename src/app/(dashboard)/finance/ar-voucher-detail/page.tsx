...
interface VoucherRow {
  ...
  customerId: string; customerName: string; customerCode: string; orderNo: string
  amount: number; paidAmount: number; balance: number; status: string; agingDays: number | null
}
                    ...
                    {data.rows.map(row => {
                      ...
                      return (
                        <TableRow key={row.id} className={`text-sm hover:bg-slate-50/40 ${row.status === 'PAID' ? 'opacity-60' : ''}`}>
                          ...
                          <TableCell>
                            <Link href={`/finance/party-transactions?partyType=CUSTOMER&partyId=${row.customerId}`} className="text-blue-600 hover:underline">
                              {row.customerName}
                            </Link>
                          </TableCell>
