...
export interface MonthRow { month: number; count: number; totalAmount: number; avgAmount: number }
export interface PayData { year: number; type: string; grandTotal: number; totalCount: number; rows: MonthRow[] }

const MONTH_LABELS = ['', '1月', '2月', '3月', '4月', '5月', '6月', '7月', '8月', '9月', '10月', '11月', '12月']
function fmt(n: number) { return n.toLocaleString('zh-TW', { minimumFractionDigits: 0, maximumFractionDigits: 0 }) }

export const TYPE_CONFIG: Record<string, { label: string; title: string; color: string; barColor: string }> = {
...
export function PaymentSummaryView({ fixedType }: { fixedType?: string }) {
  const { dict } = useI18n()
  const [year, setYear] = useState(new Date().getFullYear())
  const [type, setType] = useState(fixedType ?? 'OUTGOING')
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-slate-900">{TYPE_CONFIG[type]?.title ?? '收付款合計表'}</h1>
