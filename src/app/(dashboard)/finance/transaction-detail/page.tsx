  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-slate-900">{fp.txTitle}</h1>
          <p className="text-sm text-muted-foreground">{fp.txDesc}</p>
        </div>
      </div>
      <div className="flex flex-wrap items-end gap-3 rounded-lg border bg-white p-4">
        ...
        <div className="space-y-1">
          ...
          <select value={status} onChange={e => setStatus(e.target.value)} className="rounded-md border px-3 py-2 text-sm">
            <option value="POSTED">{fp.txStatusPosted}</option>
            <option value="DRAFT">{fp.txStatusDraft}</option>
        ...
        <div className="space-y-1">
          ...
          <select value={entryType} onChange={e => setEntryType(e.target.value)} className="rounded-md border px-3 py-2 text-sm">
            <option value="">{dict.common.all}</option>
            <option value="MANUAL">{fp.txTypeManual}</option>
            <option value="AUTO">{fp.txTypeAuto}</option>
            <option value="ADJUSTMENT">{fp.txTypeAdjustment}</option>
            <option value="CLOSING">{fp.txTypeClosing}</option>
          </select>
        </div>
      ...
      {data && (
        <>
          <div className="flex items-center justify-between text-sm text-muted-foreground">
            <span>{fp.txPagination.replace('{total}', String(data.pagination.total)).replace('{page}', String(data.pagination.page)).replace('{totalPages}', String(data.pagination.totalPages))}</span>
          ...
          <div className="rounded-lg border bg-white overflow-hidden">
            {data.data.length === 0 ? (
              <div className="py-16 text-center text-muted-foreground">{fp.txNoRecords}</div>
            ) : data.data.map(entry => {
              ...
              return (
                <div key={entry.id} className="border-b last:border-0">
                  ...
                  >
                    ...
                    <Badge className={`text-xs flex-shrink-0 ${STATUS_COLORS[entry.status] ?? ''}`}>{entry.status === 'POSTED' ? fp.txStatusPosted : entry.status === 'DRAFT' ? fp.txStatusDraft : fp.txStatusReversed}</Badge>
                    <Badge className={`text-xs flex-shrink-0 ${TYPE_COLORS[entry.entryType] ?? ''}`}>{entry.entryType === 'MANUAL' ? fp.txTypeManual : entry.entryType === 'AUTO' ? fp.txTypeAuto : entry.entryType === 'ADJUSTMENT' ? fp.txTypeAdjustment : fp.txTypeClosing}</Badge>
                  ...
                  {isOpen && (
                    <div className="bg-slate-50 border-t px-4 py-3">
                      <Table>
                        <TableHeader>
                          <TableRow className="text-xs">
                            ...
                            <TableHead className="h-8 w-24">{fp.txColAccountCode}</TableHead>
                            <TableHead className="h-8">{fp.txColAccountName}</TableHead>
                            <TableHead className="h-8 w-16">{fp.txColType}</TableHead>
                            <TableHead className="h-8">{fp.txColSummary}</TableHead>
                            <TableHead className="h-8 text-right w-28">{fp.txColDebit}</TableHead>
                            <TableHead className="h-8 text-right w-28">{fp.txColCredit}</TableHead>
                          </TableRow>
                        </TableHeader>
                        <TableBody>
                          {entry.lines.map(line => (
                            <TableRow key={line.lineNo} className="text-xs hover:bg-white/60">
                              ...
                              <TableCell className="py-1.5 text-muted-foreground">{({ ASSET: fp.txAccountTypeAsset, LIABILITY: fp.txAccountTypeLiability, EQUITY: fp.txAccountTypeEquity, REVENUE: fp.txAccountTypeRevenue, EXPENSE: fp.txAccountTypeExpense } as Record<string, string>)[line.accountType] ?? line.accountType}</TableCell>
                          ...
                          <TableRow className="border-t bg-white font-semibold text-xs">
                            <TableCell colSpan={5} className="py-1.5">{fp.txColTotal}</TableCell>
                      ...
                      {entry.referenceType && (
                        <p className="text-xs text-muted-foreground mt-2">{fp.txSourceLabel}{entry.referenceType} #{entry.referenceId}</p>
                      )}
                    </div>
                  )}
                </div>
